...
class CommandLineInterface 
    ...
    def make_a_review()
        ...
        puts ""
        reviews = Review.where(viewer_id: @user.id)
        ...
        if !@movie
            ...
            new_review = Review.create(rating: "#{rating}", fave_scene: "#{scene}", movie_id: @movie.id, viewer_id: @user.id)
            new_review.save
        end
    end
    ...
    def update_a_review
        ...
        if reviews.empty?
            puts "Sorry, you have no reviews to update at this time."
            greet
        else
        reviews.each do |review| 
    ...
    def delete_a_review
        ...
        if reviews.empty?
            puts "Sorry, you have no reviews to delete at this time."
            greet
        else   
        reviews.each do |review| 
            puts "movie: #{review.movie.title} id: #{review.id}"
